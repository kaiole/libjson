...
void expect(const json_value& value, std::string_view expected) {
    require(libjson::serialize(value) == expected, "incorrect serialized output");
}
...
void strings() {
    ...
    expect(json_value {.data = std::string {"hello world"}}, R"("hello world")");
...
void controls() {
    ...
    for (std::size_t byte = 0; byte < 32; ++byte) {
        const json_value value {.data = std::string(1, static_cast<char>(byte))};
        expect(value, "\"" + std::string(escapes[byte]) + "\"");
        require(std::get<std::string>(round_trip(value).data)
                    == std::get<std::string>(value.data), "control byte round trip failed");
    }
}
...
void invalid_utf8() {
    for (auto bytes : {"\x80", "\xbf", "\xc0\x80", "\xc1\xbf", "\xc2",
        ...
        expect_invalid_utf8(json_value {.data = json_value::object {{"key", invalid}}});
        expect_invalid_utf8(json_value {.data = json_value::object {{bytes, json_value {}}}});
    }
...
void arrays() {
    ...
    expect(json_value {.data = json_value::array {json_value {.data = true}}}, "[true]");
    const json_value value {.data = json_value::array {
        json_value {}, json_value {.data = true}, json_value {.data = false},
        ...
        json_value {.data = json_value::array {}}, json_value {.data = json_value::object {}}
    }};
    expect(value, R"([null,true,false,"text\n",-42,42,[true],[],{}])");
    require(std::get<json_value::array>(value.data).size() == 9, "input array modified");
...
void objects() {
    ...
    expect(json_value {.data = json_value::object {{"", json_value {}}}}, R"({"":null})");
    expect(json_value {.data = json_value::object {{"\"\\\n", json_value {.data = true}}}},
           R"({"\"\\\n":true})");
    expect(json_value {.data = json_value::object {{std::string("a\0b", 3), json_value {}}}},
           R"({"a\u0000b":null})");
    expect(json_value {.data = json_value::object {{"\xc3\xa9", json_value {}}}},
           "{\"\xc3\xa9\":null}");

    const json_value value {.data = json_value::object {
        {"a", json_value {.data = true}}, {"b", json_value {.data = false}}
    }};
    const auto output = libjson::serialize(value);
    require(output == R"({"a":true,"b":false})" || output == R"({"b":false,"a":true})",
            "incorrect object members or separators");
    const json_value nested {.data = json_value::object {
        {"outer", json_value {.data = json_value::array {
            json_value {.data = json_value::object {{"inner", json_value {.data = true}}}}
        }}}
    }};
...
void round_trips() {
    const json_value original {.data = json_value::object {
        {"values", json_value {.data = json_value::array {
            json_value {}, json_value {.data = true},
            json_value {.data = std::numeric_limits<std::int64_t>::min()},
            json_value {.data = std::numeric_limits<std::uint64_t>::max()},
    ...
    require(std::holds_alternative<std::monostate>(array.at(0).data), "null changed");
    require(std::get<bool>(array.at(1).data), "boolean changed");
    require(std::get<std::int64_t>(array.at(2).data) == std::numeric_limits<std::int64_t>::min(),
            "signed integer changed");
    require(std::get<std::uint64_t>(array.at(3).data) == std::numeric_limits<std::uint64_t>::max(),
            "unsigned integer changed");
    require(std::get<std::string>(array.at(4).data) == std::string("x\0\n", 3), "string changed");
    // JSON does not preserve the signed variant alternative for nonnegative integers.
    require(std::get<std::uint64_t>(round_trip(json_value {.data = std::int64_t {42}}).data) == 42,
...
int main(int argc, char* argv[]) {
    const std::pair<std::string_view, void (*)()> tests[] {
        {"literals", literals}, {"integers", integers}, {"strings", strings},
        {"controls", controls}, {"utf8", utf8}, {"invalid_utf8", invalid_utf8},
        {"arrays", arrays}, {"objects", objects}, {"round_trips", round_trips}
    };
    bool found = false;
    int failures = 0;
